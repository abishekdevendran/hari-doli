<script lang="ts">
	...
	import { Droplets, Heart as HeartIcon, Play } from 'lucide-svelte';
	...
	const memories = [
		...
		},
		{
			id: 6,
			phase: 3,
			src: '/images/asset-6.jpg',
			type: 'image',
			caption: 'Building history.',
			x: '80%',
			y: '36%'
		},
		{
			id: 7,
			phase: 4,
			src: '/images/asset-7.jpg',
			type: 'image',
			caption: 'Perfectly interwoven.',
			x: '28%',
			y: '58%'
		},
		{ id: 8, phase: 4, src: '/images/asset-8.jpg', caption: 'Deep laughter.', x: '72%', y: '58%' },
		{
			id: 9,
			phase: 5,
			src: '/images/asset-9.jpg',
			type: 'image',
			caption: 'Magnificent growth.',
			x: '50%',
			y: '58%'
		},
		{
			id: 10,
			phase: 5,
			src: '/images/asset-10.jpg',
			type: 'image',
			caption: 'Endless bloom.',
			x: '50%',
			y: '16%'
		}
	];
...
<div
	class="fixed inset-0 z-[100] hidden flex-col items-center justify-center bg-slate-900 p-6 text-center text-white landscape:flex sm:landscape:hidden"
>
	<HeartIcon class="mb-4 animate-pulse text-pink-500" size={48} />
	<h2 class="font-serif text-2xl">Please Rotate Your Device</h2>
	<p class="mt-2 text-slate-300">This experience is designed to be held upright.</p>
</div>

<main
	class="relative h-[100dvh] w-full overflow-hidden transition-colors duration-1000 {currentBg}"
>
	{#if phase >= 4}
		<div
		...
		>
			<svg
				viewBox="0 0 24 24"
				class="w-[140vw] max-w-[800px] -translate-y-10 opacity-60 drop-shadow-[0_0_30px_rgba(255,255,255,0.8)]"
			>
				<path
	...
	<div class="absolute bottom-0 left-0 flex h-full w-full flex-col items-center justify-end pb-32">
		<div class="relative flex h-[550px] w-[320px] flex-col items-center justify-end pb-8">
			{#each visibleMemories as memory (memory.id)}
				<button
					class="absolute z-30 flex h-14 w-14 -translate-x-1/2 -translate-y-1/2 items-center justify-center overflow-hidden rounded-full border-[2.5px] border-white bg-white/20 shadow-[0_0_15px_rgba(255,255,255,0.8)] backdrop-blur-sm transition-all hover:scale-110 active:scale-95"
				...
				>
					{#if memory.type === 'video'}
						...
						>
							<Play size={18} class="pointer-events-none ml-0.5 fill-white text-white opacity-90" />
						</div>
					{:else}
						<img
							src={memory.src}
			...
			<div class="relative flex flex-col items-center justify-end">
				<div class="relative z-10 flex items-end justify-center">
					<div
						class="absolute bottom-full mb-1 h-6 w-6 rounded-full bg-[#0a7a53] transition-all duration-1000 {phase >=
						1
							? 'scale-100 opacity-100'
							: 'scale-0 opacity-0'}"
					></div>
					<div
						class="absolute bottom-full mb-2 h-20 w-28 rounded-full bg-[#0a7a53] transition-all duration-1000 {phase >=
						2
							? 'scale-100 opacity-100'
							: 'scale-0 opacity-0'}"
					></div>
					...
					<div
						class="absolute bottom-full mb-2 h-64 w-64 rounded-full bg-[#0a7a53] shadow-inner transition-all duration-1000 {phase >=
						3
							? 'scale-100 opacity-100'
							: 'scale-0 opacity-0'}"
					></div>
					...
					{#if phase === 5}
						<div
							class="absolute bottom-[230px] left-1/2 z-20 flex h-0 w-0 items-center justify-center"
						>
							...
							<div
								class="absolute -top-8 -left-20"
								in:scale={{ duration: 800, start: 0.2, delay: 200 }}
							>
								<FlowerBloom color="text-rose-400" size="w-20 h-20" />
							</div>
							<div
								class="absolute -top-6 right-16"
								in:scale={{ duration: 800, start: 0.2, delay: 400 }}
							>
								<FlowerBloom color="text-pink-400" size="w-24 h-24" />
							</div>
							<div
								class="absolute top-2 -left-28"
								in:scale={{ duration: 800, start: 0.2, delay: 600 }}
							>
								<BerryCluster color="text-red-400" size="w-16 h-16" />
							</div>
							<div
								class="absolute top-6 right-24"
								in:scale={{ duration: 800, start: 0.2, delay: 800 }}
							>
			...
			<div
				class="absolute bottom-0 z-20 h-16 w-48 rounded-t-[100px] bg-[#2a2a2a] shadow-lg transition-all duration-1000"
			></div>
		</div>
		...
		<div class="absolute bottom-10 flex w-full flex-col items-center gap-4">
			...
			{:else}
				<div class="text-center text-white" transition:fade={{ delay: 500 }}>
					<h2
						class="mb-2 text-4xl font-bold text-white drop-shadow-md"
